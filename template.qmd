...
seeds_2 <- seeds_df |>
  mutate(kernal_area = l_k*w_k)

plot2 <- ggplot(seeds_2, aes(kernal_area, asym, color=variety)) +
  geom_point()
...
```{r}
#| fig-cap: Knee plot showing the number of clusters and total observations within each cluster. 

fviz_nbclust(seeds_scaled, FUNcluster = kmeans, method = 'wss', k.max = 10)
...
#R is doing this for us, we think there are 2-10 clusters within our data 

number_est <- NbClust(seeds_scaled, min.nc = 2, max.nc = 10, method = "kmeans")
...
set.seed(10101)
seeds_km <- kmeans(seeds_scaled, 3, nstart=50) # kmeans specifying 3 groups to start
...
seeds_cluster2 <- seeds_cluster |>
  mutate(area=l_k*w_k)

plot4 <- ggplot(seeds_cluster2, aes(area, asym, color=cluster_no, shape=variety)) +
  geom_point()+
  theme_bw()
...
```{r}
#| tbl-cap: Contiency table for the grouping of kmeans and the varietals of kernels.
...
```
Write psedupcode for what we will need to do for hierarchal clustering
...
6. run hclust
7. linkages either single or complex
...
#this adds row names temporarily, and keeps the matrix 
...
p_complete + p_single + plot_layout(axes = "collect")
```
